...
/**
 * Unit tests for RegulationPdfGenerator
 *
 * ทดสอบการสร้าง PDF หนังสือระเบียบ
 */
@ExtendWith(MockitoExtension.class)
@MockitoSettings(strictness = Strictness.LENIENT)
@DisplayName("RegulationPdfGenerator Tests")
class RegulationPdfGeneratorTest {

    @Mock
    private MemoPdfGenerator memoPdfGenerator;

    private RegulationPdfGenerator generator;

    @BeforeEach
    void setUp() throws Exception {
        when(memoPdfGenerator.generateMemoPdf(any())).thenReturn(createMockBase64Pdf());
        generator = new RegulationPdfGenerator(memoPdfGenerator);
    }
    ...
    class BasicConfigurationTests {
        ...
        @Test
        @DisplayName("Should return correct BookType.REGULATION")
        void shouldReturnCorrectBookType() {
            assertEquals(BookType.REGULATION, generator.getBookType());
        }

        @Test
        @DisplayName("Should return correct generator name")
        void shouldReturnCorrectGeneratorName() {
            assertEquals("RegulationPdfGenerator", generator.getGeneratorName());
        }
    }
    ...
    class PdfGenerationTests {
        ...
        void shouldGeneratePdfWithMinimalRequest() throws Exception {
            ...
            assertEquals(2, results.size()); // 1 regulation + 1 memo
        }
        ...
        void shouldGeneratePdfWithDocumentData() throws Exception {
            ...
            assertEquals("หนังสือระเบียบ", results.get(0).getDescription());
    ...
    private GeneratePdfRequest createMinimalRequest() {
        ...
        doc.setBookTitle("ทดสอบหนังสือระเบียบ");
    ...
    private GeneratePdfRequest createRequestWithDocumentData() {
        ...
        GeneratePdfRequest.BookContent bookContent = new GeneratePdfRequest.BookContent();
        bookContent.setContent("เนื้อหาทดสอบหนังสือระเบียบ");
        doc.setBookContent(bookContent);
