...
/**
 * Unit tests for AnnouncementPdfGenerator
 *
 * ทดสอบการสร้าง PDF หนังสือประกาศ
 */
@ExtendWith(MockitoExtension.class)
@MockitoSettings(strictness = Strictness.LENIENT)
@DisplayName("AnnouncementPdfGenerator Tests")
class AnnouncementPdfGeneratorTest {

    @Mock
    private MemoPdfGenerator memoPdfGenerator;

    private AnnouncementPdfGenerator generator;

    @BeforeEach
    void setUp() throws Exception {
        when(memoPdfGenerator.generateMemoPdf(any())).thenReturn(createMockBase64Pdf());
        generator = new AnnouncementPdfGenerator(memoPdfGenerator);
    }
    ...
    class BasicConfigurationTests {
        ...
        @Test
        @DisplayName("Should return correct BookType.ANNOUNCEMENT")
        void shouldReturnCorrectBookType() {
            assertEquals(BookType.ANNOUNCEMENT, generator.getBookType());
        }

        @Test
        @DisplayName("Should return correct generator name")
        void shouldReturnCorrectGeneratorName() {
            assertEquals("AnnouncementPdfGenerator", generator.getGeneratorName());
        }
    }
    ...
    class PdfGenerationTests {
        ...
        void shouldGeneratePdfWithMinimalRequest() throws Exception {
            ...
            assertEquals(2, results.size()); // 1 announcement + 1 memo
        }
        ...
        void shouldGeneratePdfWithDocumentData() throws Exception {
            ...
            assertEquals("หนังสือประกาศ", results.get(0).getDescription());
        ...
        @Test
        @DisplayName("Should generate PDF with announcement date")
        void shouldGeneratePdfWithAnnouncementDate() throws Exception {
            // Arrange
            GeneratePdfRequest request = createMinimalRequest();
            request.getDocument().setDateThai("1 มกราคม 2568");
    ...
    class EdgeCasesTests {
        ...
        @Test
        @DisplayName("Should use divisionName when department is null")
        void shouldUseDivisionNameWhenDepartmentIsNull() throws Exception {
            // Arrange
            GeneratePdfRequest request = createMinimalRequest();
            request.getDocument().setDepartment(null);
            request.getDocument().setDivisionName("ฝ่ายทดสอบ");
    ...
    private GeneratePdfRequest createMinimalRequest() {
        ...
        doc.setBookTitle("ทดสอบหนังสือประกาศ");
    ...
    private GeneratePdfRequest createRequestWithDocumentData() {
        ...
        GeneratePdfRequest.BookContent bookContent = new GeneratePdfRequest.BookContent();
        bookContent.setContent("เนื้อหาทดสอบหนังสือประกาศ");
        doc.setBookContent(bookContent);
